  ...
  def getGraph(
    ...
    case Some(g) =>
      ...
      if (!OdinConfig.VALID_GRAPHS.contains(graph))
        throw OdinException(s"'$graph' is not a valid graph type. Valid options: ${OdinConfig.VALID_GRAPHS.map(s => s"'$s'").mkString(", ")}")
      graph
  }

  // StrSubstitutor doesn't support whitespace in var (ex. ${ varName } )
  private def cleanVar(s: String): String = {
    val clean = s.replaceAll("\\$\\{\\s+", "\\$\\{").replaceAll("\\s+\\}", "\\}")
    clean
  }
    ...
    ruleType match {
      ...
      case "dependency" =>
        new Rule(
          ...
          action,
          pattern,
          updatedConfig.copy(graph = OdinConfig.DEFAULT_GRAPH)
        )
      case "token" =>
        new Rule(name, labels, ruleType, unit, priority, keep, action, pattern, updatedConfig)
  ...
  private def readRules(
    ...
    rules.asScala.toSeq.flatMap { r =>
      ...
      if (m contains "import") {
        ...
        val expand: String => Seq[String] = label =>
          config.taxonomy match {
