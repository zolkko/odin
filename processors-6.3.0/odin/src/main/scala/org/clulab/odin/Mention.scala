...
trait Mention extends Equals with Ordered[Mention] with Serializable {
  ...
  def compare(that: Mention): Int = {
    require(this.document == that.document, "can't compare mentions if they belong to different documents")
...
class EventMention(
  ...
  def this(
    ...
    this(
      Seq(label),
      mkTokenInterval(trigger, arguments),
      trigger,
      arguments,
      paths,
      sentence,
  ...
  def this(
    ...
    this(
      ...
      trigger,
      arguments,
      Map.empty[String, Map[Mention, SynPath]],
      sentence,
      document,
      keep,
      foundBy,
