    ...
    identifier ~ ":" ~ identifier ~ opt(
      ...
      case name ~ ":" ~ label ~ Some("{" ~ Some(minRep: Int) ~ "," ~ None ~ "}") ~ "=" ~ pat =>
        new ArgumentPattern(
          ...
          pat,
          required = true,
          quantifier = RangedQuantifier(minRepeat = Some(minRep), maxRepeat = None),
          config
        )
      case name ~ ":" ~ label ~ Some("{" ~ None ~ "," ~ Some(maxRep: Int) ~ "}") ~ "=" ~ pat =>
        new ArgumentPattern(
          ...
          pat,
          required = false,
          quantifier = RangedQuantifier(minRepeat = None, maxRepeat = Some(maxRep)),
          config
        )
...
class OutgoingGraphPattern(matcher: StringMatcher) extends GraphPatternNode with Graph {
...
class IncomingGraphPattern(matcher: StringMatcher) extends GraphPatternNode with Graph {
...
class ConcatGraphPattern(lhs: GraphPatternNode, rhs: GraphPatternNode) extends GraphPatternNode {
...
class DisjunctiveGraphPattern(lhs: GraphPatternNode, rhs: GraphPatternNode) extends GraphPatternNode {
...
class TokenConstraintGraphPattern(constraint: TokenConstraint) extends GraphPatternNode {
...
class LookaroundGraphPattern(lookaround: GraphPatternNode, negative: Boolean) extends GraphPatternNode {
...
class OptionalGraphPattern(pattern: GraphPatternNode) extends GraphPatternNode {
...
class KleeneGraphPattern(pattern: GraphPatternNode) extends GraphPatternNode {
