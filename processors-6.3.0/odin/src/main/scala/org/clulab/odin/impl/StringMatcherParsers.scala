...
trait StringMatcherParsers extends RegexParsers {
  ...
  def exactStringMatcher: Parser[ExactStringMatcher] = stringLiteral ^^ { string =>
    new ExactStringMatcher(string)
  }

  // a StringMatcher that uses a regex
  def regexStringMatcher: Parser[RegexStringMatcher] = regexLiteral ^^ { regex =>
    new RegexStringMatcher(regex)
  }
  ...
  def quotedStringLiteral: Parser[String] =
    """'[^\\']*(?:\\.[^\\']*)*'|"[^\\"]*(?:\\.[^\\"]*)*"""".r ^^ { s =>
      """\\(.)""".r.replaceAllIn(s.drop(1).dropRight(1), m => m.group(1))
    }

  // match a perl style "/" delimited regular expression
  // "\" is the escape character, so "\/" becomes "/"
  def regexLiteral: Parser[Regex] = """/[^\\/]*(?:\\.[^\\/]*)*/""".r ^^ { s =>
    s.drop(1).dropRight(1).replaceAll("""\\/""", "/").r
  }
