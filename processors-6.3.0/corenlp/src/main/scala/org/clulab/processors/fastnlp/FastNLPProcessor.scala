...
/**
  ...
  * Date: 1/4/14
  */
class FastNLPProcessor(internStrings: Boolean = true, withChunks: Boolean = true, withDiscourse: Int = ShallowNLPProcessor.NO_DISCOURSE)
  ...
  private def parseWithStanford(doc: Document, annotation: Annotation) {
    ...
    for (sa <- sas) {
      ...
      val collapsedDeps = SemanticGraphFactory.makeFromTree(
        gs,
        SemanticGraphFactory.Mode.CCPROCESSED,
