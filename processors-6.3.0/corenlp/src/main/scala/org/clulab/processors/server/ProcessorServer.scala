...
class ProcessorServer(
  ...
  private val procPool: ActorRef =
    system.actorOf(ProcessorActor.props(processor).withRouter(FromConfig.withSupervisorStrategy(restartEachStrategy)), "procActorPool")

  logger.debug(s"(ProcessorServer.ctor): procPool=${procPool}")

  /** A process to finally stop the server after the router dies. */
  private val serverDeathWatcher = system.actorOf(ServerDeathWatchActor.props(system, procPool), "serverDeathWatcher")
