...
class RuleBasedEntityFinder(
  ...
  def extract(doc: Document): Seq[Mention] = {
    ...
    val baseEntities = entityEngine.extractFrom(doc, stateFromAvoid).filter { entity =>
      !stateFromAvoid.contains(entity)
    }
    ...
    val res = if (avoid.isEmpty) {
      ...
      distinctEntities.filter { m =>
        stateFromAvoid.mentionsFor(m.sentence, m.tokenInterval, avoidLabel).isEmpty
      }
    }
    res
  }
