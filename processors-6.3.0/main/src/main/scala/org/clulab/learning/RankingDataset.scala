    ...
    for (d <- queryDatums) {
      d match {
        case rd: RVFDatum[Int, F] => {
          ...
          b += new Tuple3[Int, Array[Int], Array[Double]](rd.label, fvs.map(fv => fv._1), fvs.map(fv => fv._2))
        }
...
object RVFRankingDataset {
  ...
  def saveToSvmRankFormat(queries: Iterable[Iterable[Datum[Int, String]]], featureLexicon: Lexicon[String], fn: String) {
    var qid = 0
    ...
    for (d <- queryDatums) {
      d match {
        case rd: RVFKDatum[Int, F] => {
          val fvs = featuresCounterToArray(d.featuresCounter)
          b += new Tuple4[Int, Array[Int], Array[Double], String](rd.label, fvs.map(fv => fv._1), fvs.map(fv => fv._2), rd.kernel)
        }
