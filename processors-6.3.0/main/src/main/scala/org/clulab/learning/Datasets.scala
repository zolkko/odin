...
object Datasets {
  ...
  def incrementalFeatureSelection[L, F](
    ...
    while (meatLeftOnTheBone) {
      ...
      val scores = workingGroups.map(scoreGroup(_, featureGroups, chosenFeatures, dataset, classifierFactory, numFolds, scoringMetric)).toList
    ...
    logger.info(
      s"Iteration #$iteration: process ended with score $bestScore using ${chosenGroups.size} chosen groups and ${chosenFeatures.size} chosen features."
    )
  ...
  /**
    ...
    */
  def crossValidate[L, F](dataset: Dataset[L, F], classifierFactory: () => Classifier[L, F], numFolds: Int = 5): Iterable[(L, L)] = {
