...
class RFClassifier[L, F](
  ...
  def train(dataset: CounterDataset[L, F], indices: Array[Int]): Unit = {
    ...
    for (i <- 0 until numTrees) {
      bags += mkBag(
        ...
        thresholds,
        bagSize,
        RFClassifier.entropy(RFClassifier.labelCounts(indices, dataset)),
        randomSeed,
        i
      )
    }
  ...
  def buildTree(job: RFJob[L, F]): RFTree = {
    ...
    //
    val currentFeatureIndices = randomFeatureSelection(job.features, job.dataset.featureLexicon.size, job.random)
    ...
    for (f <- currentFeatureIndices) {
      val utility = featureUtility(
        f,
        job.featureThresholds(f),
        contingencyTables(f),
        job.activeNodes,
        job.currentUtility
      )
...
case class Utility(
    feature: Int, // index of this feature in the feature lexicon
    ...
    rightCounter: Counter[Int]
) // label distribution for datums with feature > threshold
