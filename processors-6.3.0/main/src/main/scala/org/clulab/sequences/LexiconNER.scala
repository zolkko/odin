...
class LexiconNER private (
  ...
  protected def findAt(seq: Array[String], caseInsensitiveSeq: Array[String], matcher: HashTrie, offset: Int, validator: EntityValidator): Int = {
...
object LexiconNER {
  ...
  def apply(
    ...
    if (overrideKBs.isDefined) {
      overrideKBs.get.foreach(okb => {
        ...
        for (name <- overrideMatchers.keySet.toList.sorted) {
          ...
          logger.info(
            s"Loaded OVERRIDE matcher for label $name. This matcher contains ${matcher.uniqueStrings.size} unique strings; the size of the first layer is ${matcher.entries.size}."
          )
    ...
    for (kb <- kbs) {
      ...
      logger.info(
        s"Loaded matcher for label $name. This matcher contains ${matcher.uniqueStrings.size} unique strings; the size of the first layer is ${matcher.entries.size}."
      )
      matchers += Tuple2(name, matcher)
      reader.close()
    }
  ...
  private def addOverrideLine(
    ...
    if (!line.startsWith("#")) { // skip comments starting with #
      ...
      if (blocks.size >= 2) {
        ...
        val matcher = matchers.getOrElseUpdate(label, new HashTrie(caseInsensitive = caseInsensitive, internStrings = INTERN_STRINGS))
  ...
  private def addWithLexicalVariations(tokens: Array[String], lexicalVariationEngine: LexicalVariations, matcher: HashTrie): Unit = {
