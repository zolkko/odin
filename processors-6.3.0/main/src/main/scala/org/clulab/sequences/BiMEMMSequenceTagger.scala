...
abstract class BiMEMMSequenceTagger[L: ClassTag, F](
  ...
  override def train(docs: Iterator[Document]): Unit = {
    ...
    if (numFoldsFirstPass > 1) {
      ...
      firstPassModel = Some(buildClassifier(sentences, mkFullFold(sentences.size), !leftToRight, None))
    }

    // make the second-pass classifier
    secondPassModel = Some(buildClassifier(sentences, mkFullFold(sentences.size), leftToRight, firstPassLabels))
  ...
  def mkFeatures(features: Counter[F], sentence: Sentence, offset: Int, history: Seq[L], firstPassLabels: Option[Array[L]]): Unit = {
    //
  ...
  def buildClassifier(sentences: ArrayBuffer[Sentence], fold: DatasetFold, leftToRight: Boolean, firstPassLabels: Option[Array[Array[L]]]): Classifier[L, F] = {

    // construct the dataset from the training partitions
    val dataset = mkDataset
    var sentCount = 0
    for {
      trainFold <- fold.trainFolds
      sentOffset <- trainFold._1 until trainFold._2
    } {
      // original sentence
  ...
  def classesOf(classifier: Classifier[L, F], origSentence: Sentence, firstPassLabels: Option[Array[L]], leftToRight: Boolean): Array[L] = {
    val sentence = if (leftToRight) origSentence else origSentence.revert()
