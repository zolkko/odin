...
trait Processor extends ProcessorAnnotator {
  ...
  def annotateList(text: List[String], keepText: Boolean = false): List[Document] = {
    val docs = ListBuffer[Document]()
    for (t <- text) {
      var doc = annotate(t, keepText)
      docs += doc
    }
    docs.toList
  }
  ...
  override def annotateFromTokens(sentences: Iterable[Iterable[String]], keepText: Boolean = false): Document = {
