...
class EDUClassifier {
  ...
  def test(trees: List[(DiscourseTree, Document)]) {
    ...
    logger.info(
      "Found " + tokens.size + " EDU tokens in TESTING: " +
  ...
  def featureSelectionIncremental(trees: List[(DiscourseTree, Document)], corpusStats: CorpusStats) {
    ...
    val chosenGroups = Datasets.incrementalFeatureSelection(dataset, Utils.svmFactory, simpleF1, featureGroups)
