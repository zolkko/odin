...
class DiscourseScorer {
  ...
  def score(sys: DiscourseTree, gold: DiscourseTree, score: DiscourseScore, scoreType: ScoreType.Value, verbose: Boolean = false) {
    if (verbose) {
  ...
  def score(sysLabel: String, goldLabel: String, score: DiscourseScore) {
...
case class DiscourseUnit(firstToken: TokenOffset, lastToken: TokenOffset, label: String)
