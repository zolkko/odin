...
object Files {
  ...
  def mkTmpDir(prefix: String, deleteOnExit: Boolean = true): String = {
    ...
    throw new IllegalStateException(
      "ERROR: Failed to create directory within "
  ...
  /**
    ...
    */
  def extractEntry(jarFileName: String, entryName: String, outFileName: String, deleteOnExit: Boolean = true, bufSize: Int = 131072): Unit = {
  ...
  def loadStreamFromClasspath(path: String): BufferedReader = {
    ...
    if (path.endsWith(".gz"))
      new BufferedReader(new InputStreamReader(new GZIPInputStream(new BufferedInputStream(is))))
    else
  ...
  def loadObjectStreamFromClasspath(path: String): ObjectInputStream = {
    ...
    if (path.endsWith(".gz"))
      new ObjectInputStream(new GZIPInputStream(new BufferedInputStream(is)))
    else
